...
echo "Releasing Orbit Conference SDK ${SDK_VERSION}"
...
# Now build and publish the Orbit Conference SDK and its dependencies
echo "Building and publishing the Orbit Conference SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Orbit Conference SDK + dependencies: ${SDK_VERSION}"
    popd
fi
